...
new_ld_path = ':'.join(cuda_lib_paths + ([current_ld_path] if current_ld_path else []))
...
import torchaudio
import traceback
from utils.formatter import format_existing_dataset
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title=os.environ.get("APP_NAME", "Gradio")) as demo:
        with gr.Tab("1 - Data processing"):
            ...
            processed_dataset_dir = gr.Textbox(
                label="Path to pre-processed dataset (audio files + transcripts):",
                value=args.audio_folder_path,
            )
            speaker_name_input = gr.Textbox(
                label="Speaker name to store in metadata:",
                value="coqui",
            )
            eval_percentage = gr.Slider(
                label="Evaluation split percentage:",
                minimum=0.05,
                maximum=0.5,
                step=0.01,
                value=0.15,
            )
            transcript_extension = gr.Textbox(
                label="Transcript file extension (per audio file):",
                value=".txt",
            )
            ...
            def preprocess_dataset(dataset_dir, language, speaker_name, eval_split, transcript_ext, out_path, progress=gr.Progress(track_tqdm=True)):
                clear_gpu_cache()
            
                dataset_dir = (dataset_dir or "").strip()
                if not dataset_dir:
                    return "Please provide the path to your processed dataset.", "", ""
                if not os.path.isdir(dataset_dir):
                    return f"The provided dataset directory does not exist: {dataset_dir}", "", ""

                transcript_ext = (transcript_ext or ".txt").strip()
                if not transcript_ext.startswith("."):
                    transcript_ext = f".{transcript_ext}"

                metadata_out = os.path.join(out_path, "dataset")
                os.makedirs(metadata_out, exist_ok=True)

                try:
                    train_meta, eval_meta, audio_total_size = format_existing_dataset(
                        audio_dir=dataset_dir,
                        target_language=language,
                        out_path=metadata_out,
                        eval_percentage=eval_split,
                        speaker_name=(speaker_name or "coqui").strip() or "coqui",
                        transcript_extension=transcript_ext,
                        gradio_progress=progress
                    )
                except Exception:
                    traceback.print_exc()
                    error = traceback.format_exc()
                    return f"Dataset configuration failed due to an error. Check the console for the full trace.\n Error summary: {error}", "", ""

                if not train_meta or not eval_meta:
                    return "No matching audio/transcript pairs were found in the provided directory.", "", ""
            
                minutes = audio_total_size / 60 if audio_total_size else 0
                print("Dataset metadata created!")
                return f"Dataset config files created successfully! Total audio: {minutes:.2f} minutes.", train_meta, eval_meta
    ...
    server_port = int(os.environ.get("GRADIO_SERVER_PORT", args.port))
    demo.launch(
        share=args.share,
        debug=False,

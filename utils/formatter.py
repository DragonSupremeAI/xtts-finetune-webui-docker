...
def format_audio_metadata_only(audio_files, asr_model, target_language="en", out_path=None, eval_percentage=0.15, speaker_name="coqui", gradio_progress=None):
    """
    Generate metadata files only, without processing or copying audio files.
    Perfect for when you have pre-processed audio files and just need the transcription mapping.
    """
    audio_total_size = 0
    ...
    if gradio_progress is not None:
        tqdm_object = gradio_progress.tqdm(audio_files, desc="Transcribing...")
    else:
    ...
    for audio_path in tqdm_object:
        ...
        try:
            ...
            audio_total_size += info.num_frames / info.sample_rate
        except:
            print(f"Warning: Could not get duration for {audio_path}")
            continue

        # Transcribe the audio file
        segments, _ = asr_model.transcribe(audio_path, language=target_language)
        full_text = " ".join([segment.text for segment in segments])
        full_text = multilingual_cleaners(full_text, target_language)

        # Use the original audio file path (no processing/copying)
        metadata["audio_file"].append(audio_path)
        metadata["text"].append(full_text)
        metadata["speaker_name"].append(speaker_name)

    # Create DataFrame and split into train/eval
    df = pandas.DataFrame(metadata)
    if len(df) == 0:
        return None, None, audio_total_size

    # Shuffle and split
    df_shuffled = df.sample(frac=1).reset_index(drop=True)
    num_val_samples = int(len(df_shuffled) * eval_percentage)
    
    eval_df = df_shuffled[:num_val_samples]
    train_df = df_shuffled[num_val_samples:]
    ...
    train_json_path = train_metadata_path.replace('.csv', '.json')
    eval_json_path = eval_metadata_path.replace('.csv', '.json')
    
    train_df.to_json(train_json_path, orient='records', indent=2)
    eval_df.to_json(eval_json_path, orient='records', indent=2)

    return train_metadata_path, eval_metadata_path, audio_total_size


def format_audio_list(audio_files, asr_model, target_language="en", out_path=None, buffer=0.2, eval_percentage=0.15, speaker_name="coqui", gradio_progress=None, metadata_only=False):
    audio_total_size = 0
    os.makedirs(out_path, exist_ok=True)
    ...
    # If metadata_only is True, delegate to the specialized function
    if metadata_only:
        return format_audio_metadata_only(
...
def format_existing_dataset(audio_dir, target_language="en", out_path=None, eval_percentage=0.15, speaker_name="coqui", transcript_extension=".txt", gradio_progress=None):
    """
    Build XTTS metadata files from an already processed dataset.
    Each audio file must have a matching transcript file with the same name and the provided extension (default .txt).
    """
    if out_path is None:
        raise ValueError("out_path must be provided")

    if audio_dir is None or not os.path.isdir(audio_dir):
        raise ValueError(f"Input directory does not exist: {audio_dir}")
    ...
    audio_files = list(list_audios(audio_dir))
    if not audio_files:
        return None, None, audio_total_size
    ...
    iterator = gradio_progress.tqdm(audio_files, desc="Mapping dataset...") if gradio_progress else tqdm(audio_files, desc="Mapping dataset...")

    for audio_path in iterator:
        transcript_path = os.path.splitext(audio_path)[0] + transcript_extension
        if not os.path.isfile(transcript_path):
            print(f"Skipping {audio_path} because transcript was not found at {transcript_path}")
            continue

        try:
            info = torchaudio.info(audio_path)
            audio_total_size += info.num_frames / max(info.sample_rate, 1)
        except Exception as exc:
            print(f"Warning: Could not get duration for {audio_path}: {exc}")

        with open(transcript_path, "r", encoding="utf-8") as transcript_file:
            text = transcript_file.read().strip()

        if not text:
            print(f"Skipping {audio_path} because transcript file is empty.")
            continue

        cleaned_text = multilingual_cleaners(text, target_language)
        metadata["audio_file"].append(os.path.abspath(audio_path))
        metadata["text"].append(cleaned_text)
        metadata["speaker_name"].append(speaker_name)
